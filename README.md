...
```
Note on the trained model: the fine-tuned model weights (`model.safetensors`, ~437MB) aren't included in this repo since they exceed GitHub's file size limit. To get a working model, run:
```bash
python train_model.py
```
This trains on the Civil Comments dataset (20,000 samples by default) and saves the model to `bert_toxic_model/`.
Then run the app:
```bash
python app.py
```
...
## About me
Mohit Raj, MCA graduate from RV College of Engineering. [GitHub](https://github.com/mohitrajjj) · [LinkedIn](https://linkedin.com/in/mohit-rajj)
